...
use solana::pumpfun::repo::{SummaryQuery, SummaryRepo};
use sqlx::PgPool;
...
impl FactService {
    pub fn new(pool: PgPool, summary_repo: SummaryRepo) -> Self {
        Self { pool, summary_repo }
    }
    ...
    pub async fn pumpfun_facts(&self) -> Box<[(TokenPairId, Facts)]> {
        ...
        for summary in summary_1m {
            let mut facts = Facts::new()
                ...
                .unwrap()
                .with_timeframe_value(
                    Fact::TradesBuyCount,
                    summary.trades.buy.trades,
                    Timeframe::M1,
                )
                .unwrap()
                .with_timeframe_value(
                    Fact::TradesSellCount,
                    summary.trades.sell.trades,
                    Timeframe::M1,
                )
                .unwrap();
