...
use base::model::{
    Notification, NotificationChannel, NotificationPayload, NotificationType, TokenPairId,
};
...
pub async fn main() {
    ...
    let token_pair_id = TokenPairId::from(21560);
        ...
        AppState::setup(Config {
            ...
            wallet: WalletConfig {
                secret: ConfigValue::Value(
                    env::var("WALLET_SECRET").expect("WALLET_SECRET must be set"),
                ),
            },
        })
        .await,
        Notification {
            ...
            payload: NotificationPayload(JsonValue::Object({
                ...
                map.insert(
                    ...
                    JsonValue::Number(token_pair_id.0.into()),
                );
                map
            })),
