    ...
    return (
        <>
            <div className={`relative flex ${wrapperClassName}`}>
                ...
                <input
                    {...props}
                    className={`${className} default-transition h-10 w-full flex-1 rounded-lg border bg-th-input-bkg px-3 font-mono text-base
          text-th-fgd-1 ${
          ...
          ${suffix ? 'pr-11' : ''}`}
                ...
                {suffix ? (
                    <span
                        className="absolute right-0 flex h-full items-center bg-transparent pr-2 text-xs text-th-fgd-4">
            {suffix}
          </span>
                ) : null}
                {maxLength ? (
