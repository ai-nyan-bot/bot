...
impl RefreshCandles {
    ...
    pub async fn refresh(&self) -> Vec<JoinHandle<()>> {
        ...
        for partition in Partition::enumerate() {
            ...
            result.push(tokio::spawn(async move {
                loop {
                    if let Ok(Some(mut tx)) = pool_1s.try_begin().await {
                        ...
                        log_ms!("calculate_progress_1s", partition, async {
                            repo_1s
            ...
            let pool_progress = pool.clone();
            result.push(tokio::spawn(async move {
                loop {
                    if let Ok(Some(mut tx)) = pool_progress.try_begin().await {
                        log_ms!("calculate_progress_1m", partition, async {
                            repo_progress
                                .calculate_progress_1m(&mut tx, partition)
                                .await
                                .unwrap();
                        });

                        log_ms!("calculate_progress_5m", partition, async {
                            repo_progress
                                .calculate_progress_5m(&mut tx, partition)
                                .await
                                .unwrap();
                        });

                        log_ms!("calculate_progress_15m", partition, async {
                            repo_progress
                                .calculate_progress_15m(&mut tx, partition)
                                .await
                                .unwrap();
                        });

                        log_ms!("calculate_progress_1h", partition, async {
                            repo_progress
                                .calculate_progress_1h(&mut tx, partition)
                                .await
                                .unwrap();
                        });

                        log_ms!("calculate_progress_6h", partition, async {
                            repo_progress
                                .calculate_progress_6h(&mut tx, partition)
                                .await
                                .unwrap();
                        });

                        log_ms!("calculate_progress_1d", partition, async {
                            repo_progress
                                .calculate_progress_1d(&mut tx, partition)
                                .await
                                .unwrap();
                        });
