...
impl TelegramLogin {
    pub fn from_query_string(query: String) -> Result<Self, TelegramLoginError> {
        ...
        let user_json = decoded_query
            .get("user")
            .ok_or(TelegramLoginError::InvalidUser)?
            .as_str();

        let user: TelegramUser =
            serde_json::from_str(user_json).map_err(|_| TelegramLoginError::InvalidUser)?;

        let hash = decoded_query
            .get("hash")
            .ok_or(TelegramLoginError::InvalidHash)?
            .clone();

        let auth_date = decoded_query
            .get("auth_date")
            .ok_or(TelegramLoginError::InvalidAuthDate)?
            .to_string();

        let signature = decoded_query
            .get("signature")
            .ok_or(TelegramLoginError::InvalidSignature)?
            .clone();

        let query_id = decoded_query
            .get("query_id")
            .ok_or(TelegramLoginError::InvalidQueryId)?
            .clone();
        ...
        query
            ...
            .filter_map(|pair| {
                ...
                let value = parts.next().map(|v| {
                    decode(v.replace("%5C%2F", r#"\/"#).as_str())
                        .unwrap()
                        .to_string()
                })?;
                Some((key, value))
...
fn generate_data_check_string(login: &TelegramLogin) -> String {
    ...
    let mut result = fields.into_iter().fold("".to_string(), |acc, f| {
        format!("{}{}={}\n", acc, f.key, f.value)
    });
...
mod tests {
    ...
    fn ok() {
        ...
        assert_eq!(login.signature, "HkX8UoSMG7njc50r3GDQ95XNHNqc6E0E95GGVsYbqMDObyIUL6omfTa_gkBFhXzbg8Z6KSX07Fzzd9R4WwAPAg");
        ...
        assert_eq!(
            login.hash,
            "aa7bdf3fff1121862c7a118ba15ddeca6d7299a9f9e882ad824010edceb67e6a".to_string()
        );
        assert_eq!(
    ...
    fn signature_does_not_match_bot_token() {
        ...
        assert_eq!(
            verify_telegram_user("0987654321:AAAAAAAAAAAAAAAAA", login),
            Err(VerificationFailed)
        );
    }
