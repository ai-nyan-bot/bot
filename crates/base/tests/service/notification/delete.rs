...
use base::model::{Notification, NotificationChannel, NotificationId, NotificationKind, NotificationPayload};
use base::repo::NotificationRepo;
use base::service::NotificationService;
...
async fn test_rolls_back_if_any_fails() {
    run_test_with_pool_on_empty_db(|pool| async move {
        ...
        assert_eq!(result.err().unwrap(), ServiceError::internal("test error"));
