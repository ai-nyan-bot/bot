...
async fn test_ok() {
    run_test_with_pool(|pool| async move {
        ...
        let result = test_instance.get_wallet(1).await.unwrap();
...
async fn test_wallet_not_found() {
    run_test_with_pool(|pool| async move {
        ...
        let err = test_instance.get_wallet(404).await.err().unwrap();
        assert_eq!(err, ServiceError::not_found("Wallet not found"));
    })
    .await
}
