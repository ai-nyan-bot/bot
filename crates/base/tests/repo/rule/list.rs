...
mod list_all {
    ...
    async fn test_ok() {
        run_test(|mut tx| async move {
            ...
            let result = test_instance.list_active(&mut tx, RuleQueryAll { limit: Limit::default() }).await.unwrap();
    ...
    async fn test_limit() {
        run_test(|mut tx| async move {
            ...
            let result = test_instance.list_active(&mut tx, RuleQueryAll { limit: Limit(1) }).await.unwrap();
            ...
            let result = test_instance.list_active(&mut tx, RuleQueryAll { limit: Limit(3) }).await.unwrap();
            assert_eq!(result.len(), 3);

            let result = test_instance.list_active(&mut tx, RuleQueryAll { limit: Limit(100) }).await.unwrap();
            assert_eq!(result.len(), 4);
