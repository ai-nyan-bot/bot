...
mod list_all {
    ...
    async fn test_ok() {
        run_test(|mut tx| async move {
            ...
            assert_eq!(result[1].name, "Strategy C");

            assert_eq!(result[2].id, 2);
            assert_eq!(result[2].user, 2);
            assert_eq!(result[2].name, "Strategy B");

            assert_eq!(result[3].id, 1);
            assert_eq!(result[3].user, 2);
            assert_eq!(result[3].name, "Strategy A");
        })
        .await
    }
    ...
    async fn test_limit() {
        run_test(|mut tx| async move {
            ...
            let result = test_instance.list_all(&mut tx, StrategyQueryAll { limit: Limit(3) }).await.unwrap();
            assert_eq!(result.len(), 3);

            let result = test_instance.list_all(&mut tx, StrategyQueryAll { limit: Limit(100) }).await.unwrap();
            assert_eq!(result.len(), 4);
