...
use crate::model::{Notification, NotificationChannel, NotificationId, NotificationPayload, NotificationType, UserId};
...
impl NotificationRepo {
	pub async fn create<'a>(&self, tx: &mut Tx<'a>, cmd: NotificationCreateCmd) -> RepoResult<Notification> {
		let notification_id = query("insert into nyanbot.notification (user_id, type, channel, payload) values ($1, $2, $3, $4) returning id")
			.bind(cmd.user)
			.bind(cmd.ty)
			.bind(cmd.channel)
			.bind(cmd.payload)
