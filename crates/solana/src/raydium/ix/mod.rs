...
impl SwapInstructionsBuilder {
    ...
    ) -> raydium::Result<UserAssociatedTokenAccounts> {
        let user_input_ata =
    ...
    pub fn handle_priority_fee_params(
        ...
        match priority_fee_config {
            Some(PriorityFeeConfig::FixedCuPrice(cu_price)) => {
                debug!("setting user defined cu-price: {}", cu_price);
                let compute_ix = ComputeBudgetInstruction::set_compute_unit_price(cu_price);
                self.compute_budget_instructions.push(compute_ix);
            }
            Some(PriorityFeeConfig::DynamicMultiplier(multiplier)) => {
                let priofee = multiplier.checked_mul(100_000).ok_or(MathError)?;
        ...
        let cu_limit = match compute_limits {
            ...
            Some(ComputeUnitLimits::Dynamic) => {
                ...
                result.value.units_consumed.and_then(|compute_units| {
                    u32::try_from(compute_units)
                        .ok()
                        .unwrap()
                        .checked_add(50_000)
                })
            }
            Some(ComputeUnitLimits::Fixed(cu_limits)) => Some(u32::try_from(cu_limits).unwrap()),
        };

        if let Some(cu_limit) = cu_limit {
            self.compute_budget_instructions
                .push(ComputeBudgetInstruction::set_compute_unit_limit(cu_limit));
        }

        Ok(cu_limit)
    }

    pub fn build_instructions(self) -> raydium::Result<Vec<Instruction>> {
        ...
        final_instructions.push(swap_instruction.unwrap());
