...
use crate::jupiter::repo::candle::CandleRepo;
...
previous_candles as (
    select r.* from jupiter.candle_1s_most_recent r
...
insert_current_candle as (
    ...
    do update set
        ...
        updated_at = now()
    where (
           {candle_price_table}.open is distinct from excluded.open or
           {candle_price_table}.high is distinct from excluded.high or
           {candle_price_table}.low is distinct from excluded.low or
           {candle_price_table}.close is distinct from excluded.close or
           {candle_price_table}.avg is distinct from excluded.avg or
           {candle_price_table}.amount_base_buy is distinct from excluded.amount_base_buy or
           {candle_price_table}.amount_quote_buy is distinct from excluded.amount_quote_buy or
           {candle_price_table}.volume_buy is distinct from excluded.volume_buy or
           {candle_price_table}.swap_buy is distinct from excluded.swap_buy or
           {candle_price_table}.amount_base_sell is distinct from excluded.amount_base_sell or
           {candle_price_table}.amount_quote_sell is distinct from excluded.amount_quote_sell or
           {candle_price_table}.volume_sell is distinct from excluded.volume_sell or
           {candle_price_table}.swap_sell is distinct from excluded.swap_sell
        )
    returning 1
),
...
impl CandleRepo {
    pub async fn calculate_1m<'a>(
        ...
        aggregate_candle(
            ...
            1,
            "minute",
            format!("jupiter.candle_1s_{partition}").as_str(),
            format!("jupiter.candle_1m_{partition}").as_str(),
        )
        .await
    }
    ...
    pub async fn calculate_5m<'a>(
        ...
        aggregate_candle(
            ...
            "minute",
            format!("jupiter.candle_1m_{partition}").as_str(),
            format!("jupiter.candle_5m_{partition}").as_str(),
        )
        .await
    }
    ...
    pub async fn calculate_15m<'a>(
        ...
        aggregate_candle(
            ...
            "minute",
            format!("jupiter.candle_5m_{partition}").as_str(),
            format!("jupiter.candle_15m_{partition}").as_str(),
        )
        .await
    }
    ...
    pub async fn calculate_1h<'a>(
        ...
        aggregate_candle(
            ...
            "hour",
            format!("jupiter.candle_15m_{partition}").as_str(),
            format!("jupiter.candle_1h_{partition}").as_str(),
        )
        .await
    }
    ...
    pub async fn calculate_6h<'a>(
        ...
        aggregate_candle(
            ...
            "hours",
            format!("jupiter.candle_1h_{partition}").as_str(),
            format!("jupiter.candle_6h_{partition}").as_str(),
        )
        .await
    }
    pub async fn calculate_1d<'a>(
        ...
        aggregate_candle(
            ...
            "day",
            format!("jupiter.candle_6h_{partition}").as_str(),
            format!("jupiter.candle_1d_{partition}").as_str(),
        )
        .await
    }
}
