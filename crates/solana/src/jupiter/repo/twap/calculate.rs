...
next_candle_ts as (
    select date_trunc('{time_unit}', timestamp) - (extract({time_unit} from timestamp)::int % {window}) * interval '1 {time_unit}' as ts from jupiter.candle_price_1m_{partition}
...
price_data as (
    select
        ...
        avg
    from
        jupiter.candle_price_1s_{partition}
    where
