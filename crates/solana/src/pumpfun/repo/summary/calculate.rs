...
on conflict (token_pair_id) do update set
	...
	updated_at = now()
	where (
    	{destination_table}.amount_base is distinct from excluded.amount_base or
    	{destination_table}.amount_base_change is distinct from excluded.amount_base_change or
		{destination_table}.amount_base_percent is distinct from excluded.amount_base_percent or
		{destination_table}.amount_base_buy is distinct from excluded.amount_base_buy or
		{destination_table}.amount_base_buy_change is distinct from excluded.amount_base_buy_change or
		{destination_table}.amount_base_buy_percent is distinct from excluded.amount_base_buy_percent or
		{destination_table}.amount_base_sell is distinct from excluded.amount_base_sell or
		{destination_table}.amount_base_sell_change is distinct from excluded.amount_base_sell_change or
		{destination_table}.amount_base_sell_percent is distinct from excluded.amount_base_sell_percent or
		{destination_table}.amount_quote is distinct from excluded.amount_quote or
		{destination_table}.amount_quote_change is distinct from excluded.amount_quote_change or
		{destination_table}.amount_quote_percent is distinct from excluded.amount_quote_percent or
		{destination_table}.amount_quote_buy is distinct from excluded.amount_quote_buy or
		{destination_table}.amount_quote_buy_change is distinct from excluded.amount_quote_buy_change or
		{destination_table}.amount_quote_buy_percent is distinct from excluded.amount_quote_buy_percent or
		{destination_table}.amount_quote_sell is distinct from excluded.amount_quote_sell or
		{destination_table}.amount_quote_sell_change is distinct from excluded.amount_quote_sell_change or
		{destination_table}.amount_quote_sell_percent is distinct from excluded.amount_quote_sell_percent or
		{destination_table}.curve_progress_open is distinct from excluded.curve_progress_open or
		{destination_table}.curve_progress_open_change is distinct from excluded.curve_progress_open_change or
		{destination_table}.curve_progress_high is distinct from excluded.curve_progress_high or
		{destination_table}.curve_progress_high_change is distinct from excluded.curve_progress_high_change or
		{destination_table}.curve_progress_low is distinct from excluded.curve_progress_low or
		{destination_table}.curve_progress_low_change is distinct from excluded.curve_progress_low_change or
		{destination_table}.curve_progress_close is distinct from excluded.curve_progress_close or
		{destination_table}.curve_progress_close_change is distinct from excluded.curve_progress_close_change or
		...
		{destination_table}.swap_percent is distinct from excluded.swap_percent or
		{destination_table}.swap_buy is distinct from excluded.swap_buy or
		{destination_table}.swap_buy_change is distinct from excluded.swap_buy_change or
		{destination_table}.swap_buy_percent is distinct from excluded.swap_buy_percent or
