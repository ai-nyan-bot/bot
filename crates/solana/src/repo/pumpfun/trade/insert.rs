...
use common::model::{Address, AddressId, Amount, DecimalAmount, Price, PublicKey, TokenMint, TokenPairId};
...
impl<L: LoadTokenInfo> TradeRepo<L> {
    pub async fn insert_trades<'a>(&self, tx: &mut Tx<'a>, slot: SlotTrades) -> RepoResult<Count> {
        ...
        for trade in slot.trades {
            ...
            let price = Price(quote_amount.0 as f64 / base_amount.0 as f64);
