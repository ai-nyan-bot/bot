...
pub(crate) fn find_missing_mints(mints: &[TokenPairMint], token_pairs: &[TokenPair]) -> Vec<TokenPairMint> {
...
impl ReadTokenPairRepo {
    ...
    pub(crate) async fn read_token_pair_mints_from_cache<'a>(&self, tx: &mut Tx<'a>, mints: &[TokenPairMint]) -> RepoResult<Vec<TokenPair>> {
    ...
    pub(crate) async fn read_token_pair_mints_from_db<'a>(&self, tx: &mut Tx<'a>, mints: &[TokenPairMint]) -> RepoResult<Vec<TokenPair>> {
        if mints.is_empty() {
            return Ok(vec![]);
        }
