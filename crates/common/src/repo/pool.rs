...
pub async fn setup_pool(postgres_config: &PostgresConfig) -> PgPool {
    let pg_pool_options = PgPoolOptions::new()
        ...
        .before_acquire(|conn, _meta| {
            Box::pin(async move {
                sqlx::query("SET work_mem TO '256MB'").execute(&mut *conn).await?;
                Ok(true)
            })
        })
        .after_release(|conn, _meta| {
            Box::pin(async move {
                sqlx::query("SET work_mem TO '20MB'").execute(&mut *conn).await?;
                Ok(true)
            })
        })
        .acquire_timeout(Duration::from_millis(postgres_config.timeout_acquire_ms.resolve_or(5_000) as u64));
    ...
    pg_options = PgConnectOptions::log_slow_statements(pg_options, LevelFilter::Warn, Duration::from_secs(1));

    info!("Connecting to Postgres: {}:{}", pg_options.get_host(), pg_options.get_port());
